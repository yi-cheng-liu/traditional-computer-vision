...
def t1(L):
    ...
    vstack = stack arrays row wise (np.row_stack)
    hstack = stack arrays column wise
    """
    return np.vstack(L)
...
def t2(X):
    ...
    """
    evalues, evector = np.linalg.eig(X)
    min_evalues = np.argmin(evalues)
    min_evector = evector[:,min_evalues] # take the whole column
...
def t3(X):
    ...
    2) X[S] = v assigns the value v to all entires of X corresponding to
       true values of S.
    """
    S = (X < 0) # set a boolean array to true when negative
    X[S] = 0 # set the true value to 0 in X
    return X
...
def t7(X):
    ...
    """
    norm = np.linalg.norm(X, axis=1) # one row is a vector(not normalized yet)
    norm = norm.reshape(-1,1)        # reshape to (n,1)
    Y = X/norm
    return Y
...
def t8(X):
    ...
    standard_deviation = np.std(X, axis=1, keepdims=True)
...
def t9(q, k, v):
    ...
    """
    a = np.linalg.norm(k-q, axis=1)
    ans = np.sum((np.exp(-a**2)) * v.T)
    return ans
...
def t10(Xs):
    ...
    5) Our 3-line solution uses no loops, and uses the algebraic trick from the
       next problem.
    """
    C = np.array([np.mean(X, axis=0) for X in Xs])
    C_norm = np.sum(C**2, axis=1, keepdims=True)
    D = C_norm + C_norm.T - 2 * np.dot(C, C.T)
...
def t12(X, Y):
    ...
    D = norm_X + norm_Y.T - 2 * np.dot(X, Y.T)
    D = np.sqrt(np.maximum(0, D))
    return D
...
def t18(N, r, x, y):
    ...
    distance = np.sqrt((X-x)**2 + (Y-y)**2) # two norm
    I = (distance < r).astype(float)
    return I
...
def t20(N, v):
    ...
    """
    X, Y = np.meshgrid(np.arange(N), np.arange(N))
    distance = np.abs(v[0]*X + v[1]* Y + v[2])/np.sqrt(v[0]**2 + v[1]**2)
    return distance
